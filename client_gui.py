...
def load_client_config():
    default = {
        "server_port": 5000, "connect_timeout": 10,
        "reconnect_attempts": 5, "reconnect_delay": 3
    }
    if os.path.exists("config.json"):
        try:
            with open("config.json") as f:
                loaded = json.load(f).get("client", {})
...
class ChatClientNetwork:
    ...
    def connect(self, server_ip, username, password, is_reconnect=False):
        self.sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.sock.settimeout(CONNECT_TIMEOUT)
        try:
            self.sock.connect((server_ip, SERVER_PORT))
        except socket.timeout:
            raise ConnectionError(f"Connection timed out after {CONNECT_TIMEOUT}s")
        except (ConnectionRefusedError, OSError) as e:
            raise ConnectionError(f"Could not reach server: {e}")
    ...
    def _receive_loop(self):
        while self.connected:
            ...
            # Split into lines; heartbeat PINGs are answered transparently
            # and never shown to the user (protocol-safe addition, Task 1/2).
            lines = chunk.split("\n")
            leftover = lines.pop() if not chunk.endswith("\n") else ""
            display_lines = []
            for line in lines:
                if line.strip() == "PING":
                    self.send_raw("PONG")
        ...
        if not self._manual_disconnect:
            self.msg_queue.put(("connection_lost", ""))
            threading.Thread(target=self._auto_reconnect, daemon=True).start()
        else:
            self.msg_queue.put(("disconnected", ""))

    def _auto_reconnect(self):
        """Reliability enhancement (Task 2): try to silently restore a dropped connection."""
        if self._reconnecting:
            return
        self._reconnecting = True
        for attempt in range(1, RECONNECT_ATTEMPTS + 1):
            self.msg_queue.put(("reconnecting", attempt))
            time.sleep(RECONNECT_DELAY)
            try:
                self.connect(self._server_ip, self.username, self._password, is_reconnect=True)
                self.msg_queue.put(("reconnected", ""))
                self._reconnecting = False
                return
            except ConnectionError:
                continue
        self._reconnecting = False
        self.msg_queue.put(("reconnect_failed", ""))
...
class ChatWindow:
    ...
    def _show_about(self):
        messagebox.showinfo(
            "About",
            f"GUI Chat Client\nAssignment 8 - Optimized Scalable Chat Application\nUser: {self.username}",
        )
    ...
    def _configure_tags(self):
        ...
        self.chat_area.tag_configure("warning", foreground=WARNING_AMBER,
                                      font=("Segoe UI", 9, "bold"))
        ...
        try:
            while True:
                ...
                if kind == "data":
                    ...
                    self.status_dot.itemconfig(self.dot_id, fill=OFFLINE_RED)
                    self._append_error("[SERVER] Connection lost. Attempting to reconnect...")
                elif kind == "reconnecting":
                    self.status_var.set(f"Reconnecting (attempt {payload})...")
                    self.status_dot.itemconfig(self.dot_id, fill=WARNING_AMBER)
                elif kind == "reconnected":
                    self.status_var.set("Connected")
                    self.status_dot.itemconfig(self.dot_id, fill=ONLINE_GREEN)
                    self._append_system("Reconnected to server.")
                    self.network.request_user_list()
                elif kind == "reconnect_failed":
                    self.status_var.set("Disconnected")
                    self.status_dot.itemconfig(self.dot_id, fill=OFFLINE_RED)
                    self._append_error("[SERVER] Could not reconnect. Please restart the client.")
