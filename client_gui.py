...
class ChatClientNetwork:
    ...
    def connect(self, server_ip, username):
        ...
        self.sock.recv(1024)  # "Enter Username:\n"
    ...
    def _receive_loop(self):
        while self.connected:
            try:
                ...
                if not data:
                    break
                self.msg_queue.put(("data", data.decode(errors="ignore")))
            except OSError:
                break
        self.connected = False
...
class LoginWindow:
    def __init__(self, root):
        ...
        self.pass_entry = self._labeled_entry(card, "PASSWORD (OPTIONAL)", show="*")
...
class ChatWindow:
    ...
    def _show_about(self):
        messagebox.showinfo(
            "About",
            f"GUI Chat Client\nAssignment 6 - TCP Chat Application\nUser: {self.username}",
        )
...
def main():
    ...
    server_ip, username = login.result
    login_root.destroy()
    ...
    try:
        network.connect(server_ip, username)
